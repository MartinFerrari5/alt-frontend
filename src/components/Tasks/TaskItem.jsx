...
const TaskItem = ({ task }) => {
  ...
  const { mutate: deleteTaskApi, isLoading: deleteTaskIsLoading } = deleteTaskMutation
  const { mutate: updateTaskApi, isLoading: updateTaskIsLoading } = updateTaskMutation
  ...
  const getNewStatus = useCallback(() => (task.status === 1 ? 0 : 1), [task.status])
    ...
    <>
      <tr className="border-b border-gray-200 bg-white hover:bg-gray-50">
        ...
        <td className="flex justify-end gap-2 px-4 py-5 text-right">
          <Button color="ghost" onClick={handleDeleteClick} disabled={deleteTaskIsLoading}>
            {deleteTaskIsLoading ? (
...
TaskItem.propTypes = {
  task: PropTypes.shape({
    ...
    worked_hours: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
