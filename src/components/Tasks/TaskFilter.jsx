...
const TaskFilter = ({ onFilter, currentPath }) => {
  ...
  const { companies_table, projects_table, hour_type_table, fetchOptions } = useOptionsStore();
  ...
  const getUrlFilterValues = useCallback(() => {
    ...
    return { fullname, company, project, status, startDate, endDate, hourtype };
  }, [searchParams]);
  ...
  const onSubmit = (data) => {
    const { fullname, company, project, status, startDate, endDate, hourtype } = data;
    const dateRange = startDate && endDate ? `${startDate} ${endDate}` : startDate || "";
    onFilter({ fullname, company, project, date: dateRange, status, hourtype });
  };
