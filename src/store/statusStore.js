  ...
  persist(
    (set) => ({
      ...
      fetchStatuses: async () => {
        ...
        if (role !== "admin") {
          set({ error: "No tienes permisos para obtener los estados." });
          return;
        }
        ...
        try {
          ...
          set({ error: "Error al obtener los estados.", loading: false });
        }
      },
      ...
      createStatus: async (payload) => {
        ...
        try {
          ...
          set((state) => ({ statuses: [...state.statuses, newStatus], loading: false }));
        } catch (error) {
      ...
      updateStatus: async (id, updatedData) => {
        const { role } = useAuthStore.getState();
        if (role !== "admin") {
          set({ error: "No tienes permisos para actualizar el estado." });
          return;
        }
        set({ loading: true, error: null });
        try {
      ...
      fetchStatusByTask: async (task_id) => {
        const { role } = useAuthStore.getState();
        if (role !== "admin") {
          set({ error: "No tienes permisos para obtener el estado de la tarea." });
          return;
        }
        if (!task_id) {
        ...
        try {
          ...
          set({ error: "Error al obtener el estado de la tarea.", loading: false });
        }
      },
